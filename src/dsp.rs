...
pub fn compute_signal_mean(signal_array: &[f32]) -> f32 {
    let mut mean: f32 = Default::default();

    for item in signal_array.iter() {
        mean += item;
    }

    mean / signal_array.len() as f32
}

pub fn compute_signal_variance(signal_array: &[f32]) -> f32 {
    let signal_mean = compute_signal_mean(signal_array);
    let mut signal_variance: f32 = Default::default();

    for sample in signal_array.iter() {
        signal_variance += f32::powi(sample - signal_mean, 2);
    }
    signal_variance / signal_array.len() as f32
}

pub fn compute_signal_devication(variance: f32) -> f32 {
    f32::sqrt(variance)
}


pub fn convolution(signal_array: &[f32], impulse_response:&[f32])->Vec<f32>{
    
    // 1. Generate output signal array according to the convolution equation:
    let mut convolution_result:Vec<f32> = (0..signal_array.len() + impulse_response.len()).map(|_x|{ return  0.0_f32 }).collect();
...
pub fn convolution_output_side(signal_array: &[f32], impulse_response:&[f32])->Vec<f32>{
        // 1. Generate output signal array according to the convolution equation:
    let mut convolution_result:Vec<f32> = (0..signal_array.len() + impulse_response.len()).map(|_x|{0 as f32}).collect();
...
pub fn running_sum(signal_in: &[f32])->Vec<f32>{
    let mut out_signal:Vec<f32> = Vec::new();
...
impl DftResult for DftData{
    fn as_polar(&self)->Polar {
        ...
        for i in 0..self.real_part.len(){
            ...
            if float_cmp::approx_eq!(f32,self_real,0.0, ulps=2) {
                self_real = 10.0_f32.powf(-20.0);
            }
            ...
            let is_nuisance3_only_real_negative = self_real < 0.0 &&
                ((self_im > 0.0)|| float_cmp::approx_eq!(f32,self_im,0.0));


            if is_nuisance3_both_negative {
                sample.phase-=std::f32::consts::PI;
            }
...
pub fn dft_transform(signal_array: &[f32])->DftData{
    
    let mut dft_result = DftData
    {
        real_part:(0..signal_array.len() / 2 ).map(|_x|{0 as f32}).collect(),
        im_part: (0..signal_array.len() / 2 ).map(|_x|{0 as f32}).collect()
    };
    ...
    for k in 0.. signal_array.len()/2{
        for i in 0.. signal_array.len() - 1{
            let common_part = (2.0* std::f32::consts::PI*(k as f32) *(i as f32) / (n as f32) ) as f32;
...
pub fn compute_signal_magnitude(dft_result:&DftData, signal_length:usize)->Vec<f32>{
    let mut output_mag:Vec<f32> = (0..signal_length/2).map(|_x|{0 as f32}).collect();
...
pub fn inverse_dft_transform(signal_length:usize, real_part:&[f32], im_part:&[f32])->Vec<f32>{
    let mut output_signal:Vec<f32> = (0..signal_length).map(|_x|{0 as f32}).collect();
    let mut output_rex:Vec<f32> = Vec::new();
    let mut output_imx:Vec<f32> = Vec::new();

    for k in 0..signal_length/2{
        output_rex.push(real_part[k] / signal_length as f32 / 2.0);
        output_imx.push( -im_part[k] / signal_length as f32 / 2.0);
    }
    ...
    for k in 0..real_part.len(){
        for i in 0..signal_length{
            let common_multiplier = 2.0* std::f32::consts::PI*(k as f32) *(i as f32) / (n as f32);
            output_signal[i] = output_signal[i] +  real_part[k] * common_multiplier.cos() as f32;
            output_signal[i] = output_signal[i] +  im_part[k] * common_multiplier.sin() as f32;
        }
    }
    output_signal
}

pub fn complex_dft_transform(signal_array_real: &[f32],signal_array_im: &[f32])->Vec<num::complex::Complex32>{
    let mut output_complex:Vec<num::complex::Complex32> = (0..signal_array_real.len()).map(|_x|{Complex::new(0.0,0.0)}).collect();
    ...
    for k in 0..output_complex.len(){
        ...
        for i in 0..signal_array_real.len(){
            ...
            let sr = (2.0*std::f32::consts::PI*k as f32 *i as f32 / signal_array_real.len() as f32).cos();
            let si = -(2.0*std::f32::consts::PI*k as f32 *i as f32 / signal_array_real.len() as f32 ).sin();
...
fn compute_fft_stages_order(signal_array:&[f32])->u8{
...
fn bit_reversal_in_place(signal_array:&[f32])->Vec<num::complex::Complex32>{
    let mut bit_reversed:Vec<num::complex::Complex32> = signal_array.iter().map(|real_sample|{ num::complex::Complex32::new(*real_sample, 0.0_f32) }).collect();
...
impl TwindleFactorOrdered for TwindleFactor{
    fn compute_for_order(&self)->num::complex::Complex32 {
        let real:f32 = (2.0_f32 * std::f32::consts::PI * self.index as f32 / self.order as f32).cos();
        let imag:f32 = -(2.0_f32 * std::f32::consts::PI * self.index as f32 / self.order as f32).sin();

        num::complex::Complex32::new(real,imag)
    }
}

pub fn fft_transform(signal_array:&[f32])->Vec<num::complex::Complex32>{
...
pub fn compute_hamming_window(filter_kernel_length:usize)->Vec<f32>{
    let mut filter_kernel:Vec<f32> = (0..filter_kernel_length).map(|_x|{0 as f32}).collect();
    for i in 0.. filter_kernel_length{
            let computed_sample = 2.0_f32*std::f32::consts::PI * i as f32 / filter_kernel_length as f32;
            filter_kernel[i] = 0.54_f32 -0.46_f32*computed_sample.cos();
    }
    filter_kernel
}

pub fn compute_blackman_window(filter_kernel_length:usize)->Vec<f32>{
    let mut filter_kernel:Vec<f32> = (0..filter_kernel_length).map(|_x|{0 as f32}).collect();
    for i in 0.. filter_kernel_length{
        let computed_sample = 2.0_f32*std::f32::consts::PI * i as f32 / filter_kernel_length as f32;
        let second_sample = 4.0_f32*std::f32::consts::PI * i as f32 / filter_kernel_length as f32;
        filter_kernel[i] = 0.42_f32 -0.5_f32*computed_sample.cos() + 0.08_f32*second_sample.cos();
    }
    filter_kernel
}


fn normalize_frequency(cutoff_frequency:f32,sampling_frequency:f32)->f32{
    let nyquist_frequency = sampling_frequency / 2.0_f32;
    // Propotion example
...
pub fn design_windowed_sinc_filter(cutoff_frequency:f32,sampling_frequency:f32, filter_kernel_length:usize)->Vec<f32>{
    let mut filter_kernel:Vec<f32> = (0..filter_kernel_length).map(|_x|{0 as f32}).collect();
    ...
    for i in 0..filter_kernel_length{
        if i  == (filter_kernel_length / 2){
            filter_kernel[i] = 2.0_f32*std::f32::consts::PI*normalized_cutoff_frequency*k;
        }
        else{
            let sinc_base = 2.0_f32*std::f32::consts::PI*normalized_cutoff_frequency*(i as f32 - filter_kernel_length as f32/2.0_f32) / ( i  as f32 - filter_kernel_length as f32/2.0_f32);
            filter_kernel[i] = k * sinc_base;
            filter_kernel[i]*=hamming_window[i];
        }
    }
    let normalization_factor_sum:f32 = filter_kernel.iter().sum();
...
pub fn perform_spectral_reversal(filter_kernel:&mut Vec<f32>){
    for i in 0..filter_kernel.len(){
        filter_kernel[i] = (-1.0_f32).powf(i as f32) * filter_kernel[i];
    }
}

pub fn perform_spectral_inversion(filter_kernel:&mut Vec<f32>){
    for i in 0..filter_kernel.len(){
...
pub fn design_windowed_sinc_filter_hpf(cutoff_frequency:f32,sampling_frequency:f32, filter_kernel_length:usize)->Vec<f32>{
...
pub fn design_bandbass_filter(low_bandpass_freq:f32,high_bandpass_freq:f32,sampling_frequency:f32, filter_kernel_length:usize)->Vec<f32>{
    let mut low_passband_filter_kernel:Vec<f32> = design_windowed_sinc_filter(low_bandpass_freq,sampling_frequency,filter_kernel_length);
    let mut high_passband_filter_kernel:Vec<f32> = design_windowed_sinc_filter(high_bandpass_freq, sampling_frequency, filter_kernel_length);
